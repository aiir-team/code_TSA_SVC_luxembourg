...
import os
from app_settings import IMAGES_FOLDER
...
def _handle_raw_data__(data):
    ...
    tweets = [word for word in tweets if word not in stopwords.words('english') and len(word) > 1]
    # print(len(tweets))
    # print(tweets[:100])

    # print('---remove numbers---')
    tweets = [word for word in tweets if not word.isdigit()]
    # print(len(tweets))
    # print(tweets[:100])
...
def create_dataset_and_features__(df, word_frequency):
    ...
    ## Create and save word cloud images
    all_words = ' '.join([text for text in df['tweet_content']])
    wordcloud = WordCloud(width=800, height=500, random_state=21, max_font_size=110).generate(all_words)
    plt.figure(figsize=(10, 7))
    plt.imshow(wordcloud, interpolation="bilinear")
    plt.axis('off')
    plt.savefig(os.path.join(IMAGES_FOLDER, 'word_cloud.png'))
...
def prepare_dataset(word_frequency):
    t1 = time.time()
    df = pd.read_csv('database/downloadedB.csv', delimiter='\t', encoding="ISO-8859-1", names=['id1', 'id2', 'sentiment', 'tweet_content'])
